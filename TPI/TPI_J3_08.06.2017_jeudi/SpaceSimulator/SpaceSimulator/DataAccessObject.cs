...
namespace SpaceSimulator
{
    class DataAccessObject
    {
        ...
        /// <param name="image">image à afficher</param>
        /// <param name="orbitCenterId">id du référentiel autour duquel la lune orbite</param>
        public static void InsertMoon(string name, double ray, double period, double distanceOrbitCenter, Image image, int orbitCenterId)
        {
            StartConnection();
            ...
            string SQLrequest = String.Format("INSERT INTO moons ('name', 'ray', 'period', 'distanceToOrbitCenter', 'image', 'idOrbitCenter') VALUES ('{0}', '{1}', '{2}', '{3}', @image, '{4}')",
                name, ray, period, distanceOrbitCenter, orbitCenterId);
        ...
        public static List<Moon> GetMoonsFromPlanet(Planet satellite)
        {
            ...
            string SQLrequest = String.Format("SELECT * FROM Moons WHERE idOrbitCenter = {0}", satellite.Id);
            ...
            myConnection.Close();
            foreach (Moon moon in orbitingMoons)
            {
        ...
        public static Moon GetMoonFromName(string name, Planet planet)
        {
            ...
            string SQLrequest = String.Format("SELECT * FROM moons WHERE idOrbitCenter = {0} AND Name = '{1}'", planet.Id, name);
        ...
        public static void UpdateMoon(Moon updatedMoon)
        {
            ...
            string SQLrequest = string.Format("UPDATE moons SET name = '{0}', ray='{1}', period='{2}', distanceToOrbitCenter ='{3}', image=@image, idOrbitCenter = '{4}' WHERE id = {5}",
